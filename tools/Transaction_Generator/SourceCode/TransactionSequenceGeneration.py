...
def getRandByteArray():
    random_bytes = random.randbytes(random.randint(4,32))
...
def transacSeqGenerator(F_all: dict, Max: int):
    ...
    while g < Max:  # 循环直到达到最大序列数
        ...
        while len(txs) < 5 and len(txs) < len(F_all):  # 循环直到包含所有函数
...
def getTransactionSequence(filename: str, contractsInfo: dict, Max: int):
    ...
    """
    seedPool = {}
    if 'constructor' in contractsInfo:
        constructor = generateTransaction(contractsInfo['constructor'], 0)
        ...
        with open(f"/root/Transaction_Generator/output/{filename}.json", "w") as f:
